...
func (i *ResponseInbound) TransformStream(ctx context.Context, stream *model.InternalLLMResponse) ([]byte, error) {
	...
	if stream.Object == "[DONE]" {
		...
		if i.hasFinished && !i.responseCompleted {
			for _, event := range i.emitTerminalResponseEvent() {
				if event != nil {
	...
	if len(stream.Choices) > 0 {
		...
		if choice.FinishReason != nil && !i.hasFinished {
			i.hasFinished = true
			i.terminalEventType, i.terminalStatus = responsesTerminalState(*choice.FinishReason)
			i.terminalFinishReason = *choice.FinishReason
	...
	// Handle final usage chunk after finish_reason.
	if stream.Usage != nil && i.hasFinished && !i.responseCompleted {
		events = append(events, i.emitTerminalResponseEvent()...)
	}
...
func (i *ResponseInbound) handleToolCalls(toolCalls []model.ToolCall) [][]byte {
	...
	for _, tc := range toolCalls {
		...
		if tc.Function.Arguments != "" || tc.Function.Name != "" || tc.ID != "" {
			...
			toolCallOutputIdx := i.toolCallOutputIndex[toolCallIndex]
...
func terminalIncompletionReason(finishReason string) string {
...
func (i *ResponseInbound) emitTerminalResponseEvent() [][]byte {
	if i.responseCompleted {
		return nil
	}

	eventType := i.terminalEventType
	...
	if status == "incomplete" {
		finishReason = terminalIncompletionReason(i.terminalFinishReason)
	}
	...
	if finishReason != "" {
		response.IncompleteDetails = &ResponsesIncompleteDetails{Reason: finishReason}
	}
...
type ResponsesItem struct {
	...
	// Refusal content emitted by the Responses API for rejected output.
...
func convertToInternalRequest(req *ResponsesRequest) (*model.InternalLLMRequest, error) {
	...
	if req.Text != nil && req.Text.Format != nil && req.Text.Format.Type != "" {
		...
		if len(req.Text.Format.Schema) > 0 {
			chatReq.ResponseFormat.JSONSchema = req.Text.Format.Schema
		}
	}
...
func convertToResponsesAPIResponse(resp *model.InternalLLMResponse) *ResponsesResponse {
	...
	for _, choice := range resp.Choices {
		...
		if message.Content.Content != nil && *message.Content.Content != "" {
			...
			flushTextMessage := func() {
				if len(contentItems) == 0 {
					return
				}

				messageItems := append([]ResponsesItem(nil), contentItems...)
				result.Output = append(result.Output, ResponsesItem{
					ID:      generateItemID(),
					Type:    "message",
					Role:    "assistant",
					Content: &ResponsesInput{Items: messageItems},
					Status:  lo.ToPtr("completed"),
				})
			...
			for _, part := range message.Content.MultipleContent {
				...
				case "image_url":
					if part.ImageURL != nil {
						...
						if parsed := xurl.ParseDataURL(part.ImageURL.URL); parsed != nil && parsed.IsBase64 {
							imageItem.Result = lo.ToPtr(parsed.Data)
							if slash := strings.Index(parsed.MediaType, "/"); slash >= 0 && slash < len(parsed.MediaType)-1 {
								outputFormat := parsed.MediaType[slash+1:]
								imageItem.OutputFormat = &outputFormat
							}
						} else {
							imageItem.Result = lo.ToPtr(xurl.ExtractBase64FromDataURL(part.ImageURL.URL))
						}
						...
						result.Output = append(result.Output, ResponsesItem{
							...
							OutputFormat: imageItem.OutputFormat,
		...
		if choice.FinishReason != nil {
			...
			case "length":
				...
				result.IncompleteDetails = &ResponsesIncompleteDetails{Reason: "length"}
			case "content_filter":
				result.Status = lo.ToPtr("incomplete")
				result.IncompleteDetails = &ResponsesIncompleteDetails{Reason: "content_filter"}
			case "tool_calls":
				result.Status = lo.ToPtr("completed")
			case "error":
