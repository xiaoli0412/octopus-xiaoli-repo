...
type ResponseOutbound struct {
	...
	toolCallIndexByCallID map[string]int
	toolCallStateByIndex  map[int]*streamedToolCallState
	messageStateByOutput  map[int]*streamedMessageState
	messageStateByItemID  map[string]*streamedMessageState
}
...
func (o *ResponseOutbound) resetToolCallStreamState() {
	o.nextToolCallIndex = 0
	o.toolCallIndexByOutput = make(map[int]int)
	o.toolCallIndexByItemID = make(map[string]int)
	o.toolCallIndexByCallID = make(map[string]int)
	o.toolCallStateByIndex = make(map[int]*streamedToolCallState)
}

func (o *ResponseOutbound) resetMessageStreamState() {
	o.messageStateByOutput = make(map[int]*streamedMessageState)
	o.messageStateByItemID = make(map[string]*streamedMessageState)
}

func (o *ResponseOutbound) ensureToolCallStreamState() {
	if o.toolCallIndexByOutput == nil || o.toolCallIndexByItemID == nil || o.toolCallIndexByCallID == nil || o.toolCallStateByIndex == nil {
		o.resetToolCallStreamState()
	}
}

func (o *ResponseOutbound) ensureMessageStreamState() {
	if o.messageStateByOutput == nil || o.messageStateByItemID == nil {
		o.resetMessageStreamState()
	}
}

func (o *ResponseOutbound) toolCallState(index int) *streamedToolCallState {
	o.ensureToolCallStreamState()
	if state, ok := o.toolCallStateByIndex[index]; ok {
		return state
	}

	state := &streamedToolCallState{}
	o.toolCallStateByIndex[index] = state
	return state
}

func (o *ResponseOutbound) markToolCallState(index int, itemID, callID, name, arguments string, added bool) *streamedToolCallState {
...
func (o *ResponseOutbound) fallbackToolCallChunk(index int, itemID, callID, name, arguments string) *model.InternalLLMResponse {
	...
	missingArguments := !state.ArgumentsSeen && arguments != ""

	if !missingID && !missingName && !missingArguments {
		return nil
	}
...
func (o *ResponseOutbound) toolCallIndexForStreamEvent(outputIndex int, itemID *string, callID string) int {
	...
	return o.rememberToolCallIndex(outputIndex, resolvedItemID, callID)
}
...
func (o *ResponseOutbound) TransformResponse(ctx context.Context, response *http.Response) (*model.InternalLLMResponse, error) {
	body, err := readOpenAIResponseBody(response)
	if err != nil {
		return nil, err
	}
...
func (o *ResponseOutbound) TransformStream(ctx context.Context, eventData []byte) (*model.InternalLLMResponse, error) {
	...
	case "response.function_call_arguments.delta":
		...
		o.markToolCallState(toolCallIndex, resolvedStreamItemID(streamEvent.ItemID), streamEvent.CallID, streamEvent.Name, streamEvent.Delta, false)
		resp.Choices = []model.Choice{
			{
				Index: 0,
				Delta: &model.Message{
	...
	case "response.function_call_arguments.done":
		toolCallIndex := o.toolCallIndexForStreamEvent(streamEvent.OutputIndex, streamEvent.ItemID, streamEvent.CallID)
		fallback := o.fallbackToolCallChunk(toolCallIndex, resolvedStreamItemID(streamEvent.ItemID), streamEvent.CallID, "", streamEvent.Arguments)
		if fallback == nil {
			return nil, nil
		}
		resp = fallback
	...
	case "response.incomplete":
		finishReason := "length"
		if streamEvent.Response != nil && streamEvent.Response.IncompleteDetails != nil && streamEvent.Response.IncompleteDetails.Reason == "content_filter" {
...
func resolvedStreamItemID(itemID *string) string {
	if itemID == nil {
		return ""
	}
	return *itemID
...
func extractOutputMessageText(item *ResponsesItem) string {
	...
	for _, contentItem := range item.Content.Items {
		if contentItem.Type == "output_text" && contentItem.Text != nil {
...
type ResponsesItem struct {
	...
	// Refusal content returned by the Responses API.
...
func convertToLLMResponseFromResponses(resp *ResponsesResponse) *model.InternalLLMResponse {
	...
	appendTextPart := func(text string) {
		...
		last := len(contentParts) - 1
		if contentParts[last].Type == "text" {
			if contentParts[last].Text == nil {
				contentParts[last].Text = lo.ToPtr("")
			}
			*contentParts[last].Text += text
			return
		}

		textCopy := text
		contentParts = append(contentParts, model.MessageContentPart{Type: "text", Text: &textCopy})
	}

	promotePlainText := func() {
		if plainText.Len() == 0 {
			return
		}
		...
		plainText.Reset()
		contentParts = append(contentParts, model.MessageContentPart{Type: "text", Text: lo.ToPtr(text)})
	}

	appendImagePart := func(url string) {
		if url == "" {
			return
		}
	...
	for _, outputItem := range resp.Output {
		...
		case "message":
			if outputItem.Refusal != nil && *outputItem.Refusal != "" {
			...
			if outputItem.Content != nil {
				for _, item := range outputItem.Content.Items {
					if item.Type == "output_text" && item.Text != nil {
						appendTextPart(*item.Text)
					}
					if item.Type == "input_image" && item.ImageURL != nil {
						appendImagePart(*item.ImageURL)
					}
				}
			}
		case "output_text":
			if outputItem.Text != nil {
				appendTextPart(*outputItem.Text)
			}
		case "function_call":
		...
		case "image_generation_call":
			if outputItem.Result != nil && *outputItem.Result != "" {
				...
				appendImagePart("data:image/" + outputFormat + ";base64," + *outputItem.Result)
			}
		}
	}
	...
	// Set message content
	if plainText.Len() > 0 {
		textContent.WriteString(plainText.String())
	}
	...
	if len(contentParts) > 0 {
		...
		choice.Message.Content = model.MessageContent{MultipleContent: contentParts}
	}
	...
	if len(toolCalls) > 0 {
		...
		case "incomplete":
			if resp.IncompleteDetails != nil && resp.IncompleteDetails.Reason == "content_filter" {
				choice.FinishReason = lo.ToPtr("content_filter")
			} else {
