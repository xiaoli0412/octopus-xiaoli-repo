...
func TestModelConnectivity(ctx context.Context, req model.ModelTestRequest) (model.ModelTestResult, error) {
	...
	httpClient, err := channelHttpClientForConnectivity(channel)
	if err != nil {
		return model.ModelTestResult{
			Success:      false,
...
func checkModelPriceMatch(modelName string, channelID int) bool {
	localInfo, err := LLMGet(modelName)
	if err != nil {
		return false
	}

	upstreamPrice, ok := ResolveGatewayLLMPrice(modelName, channelID)
	if !ok {
		return false
	}
...
func channelHttpClientForConnectivity(channel *model.Channel) (*http.Client, error) {
	if channel == nil {
