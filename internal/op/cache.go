...
func InitCache() error {
	...
	if err := BackfillAIProfileTypedPayloads(ctx); err != nil {
		return fmt.Errorf("ai profile typed backfill error: %v", err)
	}
	...
	if err := routeTargetRefreshCache(ctx); err != nil {
		return fmt.Errorf("route target refresh cache error: %v", err)
	}
