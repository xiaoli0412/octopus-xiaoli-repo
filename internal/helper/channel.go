...
func ChannelBaseUrlDelayUpdate(channel *model.Channel, ctx context.Context) {
	...
	for _, baseUrl := range channel.BaseUrls {
		...
		httpClient, err := channelHTTPClientForManagement(channel)
		if err != nil {
			log.Warnf("failed to get http client (channel=%d): %v", channel.ID, err)
			continue
		}
		delayCtx, cancel := context.WithTimeout(ctx, modelFetchRequestTimeout)
		delay, err := GetUrlDelay(httpClient, baseUrl.URL, delayCtx)
		cancel()
		if err != nil {
