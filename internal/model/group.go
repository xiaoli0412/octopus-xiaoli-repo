...
type Group struct {
	...
	RetryRounds       int         `json:"retry_rounds"`         // 多轮尝试次数（>=1），1 表示只跑一轮
	RetryDelayMs      int         `json:"retry_delay_ms"`       // 每轮之间延迟(ms)，0 表示不延迟
	FailoverWindowSec int         `json:"failover_window_sec"`  // 高级故障转移总窗口(秒)
	RaceAfterFails    int         `json:"race_after_fails"`     // 连续失败达到该阈值后进入并发竞速
	RaceConcurrency   int         `json:"race_concurrency"`     // 并发竞速上限
...
type GroupUpdateRequest struct {
	...
	RetryRounds       *int                     `json:"retry_rounds,omitempty"`         // 多轮尝试次数
	RetryDelayMs      *int                     `json:"retry_delay_ms,omitempty"`       // 每轮延迟(ms)
	FailoverWindowSec *int                     `json:"failover_window_sec,omitempty"`
	RaceAfterFails    *int                     `json:"race_after_fails,omitempty"`
	RaceConcurrency   *int                     `json:"race_concurrency,omitempty"`
