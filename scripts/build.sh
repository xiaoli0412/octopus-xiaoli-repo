...
# Build flags
readonly LDFLAGS="-X 'github.com/xiaoli0412/octopus-xiaoli-repo/internal/conf.Version=${GIT_VERSION}' \
                  -X 'github.com/xiaoli0412/octopus-xiaoli-repo/internal/conf.BuildTime=${BUILD_TIME}' \
                  -X 'github.com/xiaoli0412/octopus-xiaoli-repo/internal/conf.Author=${GIT_AUTHOR}' \
                  -X 'github.com/xiaoli0412/octopus-xiaoli-repo/internal/conf.Commit=${COMMIT_ID}' \
                  -s -w"
...
prepare_lightweight_environment() {
    log_step "Preparing lightweight build environment"
    log_info "Checking required commands..."
    ...
    if ! mkdir -p "${OUTPUT_DIR}/bin"; then
        log_error "Failed to create subdirectory: ${OUTPUT_DIR}/bin"
        return 1
    fi

    log_success "Lightweight build environment ready"
}
...
build_frontend() {
    ...
    # Build the project and sync the exported assets into static/out
    log_info "Building frontend project and syncing static assets..."
    if ! NEXT_PUBLIC_APP_VERSION="$GIT_VERSION" pnpm run build:static; then
...
create_archives() {
    ...
    if [ -f "${OUTPUT_DIR}/bin/SHA256SUMS" ]; then
        if cp "${OUTPUT_DIR}/bin/SHA256SUMS" "${archives_dir}/SHA256SUMS" 2>/dev/null; then
            log_success "Copied SHA256SUMS into archives/"
        else
            log_error "Failed to copy SHA256SUMS into archives/"
        fi
    else
        log_warning "SHA256SUMS not found in bin/, skipping archive checksum manifest"
    fi
...
generate_checksums() {
    ...
    if find . -maxdepth 1 -name "${APP_NAME}-*" -type f -print0 | xargs -0 $checksum_cmd >SHA256SUMS 2>/dev/null; then
        local checksum_count=$(wc -l <SHA256SUMS 2>/dev/null || echo "0")
        log_success "Generated SHA-256 checksums for ${checksum_count} files in bin/"
    else
...
show_usage() {
    ...
    echo ""
    echo "Commands:"
    echo "  linux-binary         Build only the primary Linux server binary"
    echo "  linux-server         Build the primary Linux server artifact flow"
    echo "  docker-image         Build a local Docker image from current source"
...
main() {
    case "${1:-}" in
    "linux-binary")
        log_step "Starting lightweight Linux binary build"
        echo "?? Building ${APP_NAME} ${GIT_VERSION} (${COMMIT_ID}) for linux/x86_64"
        echo ""

        if ! prepare_lightweight_environment; then
            log_error "Failed to prepare build environment"
            exit 1
        fi

        log_step "Building Linux server binary only"
        if ! build_standard linux x86_64; then
            log_error "Failed to build Linux x86_64"
            exit 1
        fi

        log_step "Lightweight Linux build completed"
        log_success "Linux binary ready: ${OUTPUT_DIR}/bin/${APP_NAME}-linux-x86_64"
        ;;
    "linux-server")
        log_step "Starting Linux server build"
        echo "📦 Building ${APP_NAME} ${GIT_VERSION} (${COMMIT_ID}) for linux/x86_64"
        echo ""
        ...
        log_step "Building Linux server binary"
        if ! build_standard linux x86_64; then
            log_error "Failed to build Linux x86_64"
            exit 1
        fi
        ...
        log_step "Linux server build completed"
        log_success "Primary server binary ready: ${OUTPUT_DIR}/bin/${APP_NAME}-linux-x86_64"
        log_info "  • Linux binary: ${OUTPUT_DIR}/bin/${APP_NAME}-linux-x86_64"
        log_info "  • Docker binary: ${OUTPUT_DIR}/docker/linux/amd64/${APP_NAME}"
        ;;
    "docker-image")
        log_step "Building local Docker image from source"
        if ! command_exists docker; then
            log_error "docker is not installed. Please install Docker Desktop or Docker Engine."
            exit 1
        fi

        if ! docker build -f scripts/dockerfiles/Dockerfile.debian -t ${APP_NAME}:local .; then
            log_error "Failed to build local Docker image"
            exit 1
        fi

        log_success "Docker image ready: ${APP_NAME}:local"
        ;;
    "build")
        if [ $# -ne 3 ]; then
