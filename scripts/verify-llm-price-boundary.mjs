...
assert.match(modelIndexSource, /const estimateItemHeight = layout === 'list' \? 92 : 156/);
assert.match(modelIndexSource, /columns=\{\{ default: 1, md: layout === 'list' \? 1 : 2, lg: layout === 'list' \? 1 : 3 \}\}/);
...
assert.match(modelItemSource, /data-slot="model-card-meta"/);
assert.match(modelItemSource, /data-slot="model-card-meta-compact"/);
...
assert.match(modelIndexSource, /data-layout=\{layout\}/);
assert.match(modelIndexSource, /renderItem=\{\(model, index\) => <ModelItem model=\{model\} layout=\{layout\} index=\{index\} \/>\}/);
...
assert.match(modelItemSource, /data-layout=\{layout\}/);
