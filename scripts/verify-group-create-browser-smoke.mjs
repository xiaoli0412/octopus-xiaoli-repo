...
const sessionName = process.env.OCTOPUS_UI_SMOKE_SESSION || `octopus-group-create-${process.pid}`;
...
function smokeLog(message, details = {}) {
  ...
  console.log(`[group-create-smoke] ${message}${suffix}`);
}
...
function printUsage() {
  console.log([
    'Usage:',
    '  node scripts/verify-group-create-browser-smoke.mjs --check-only',
    '  node scripts/verify-group-create-browser-smoke.mjs --external',
    '  node scripts/verify-group-create-browser-smoke.mjs --self-start',
    '',
...
async function seedAuthenticatedGroupPage() {
...
async function ensureGroupPageReady(timeoutMs = 45000) {
  const startedAt = Date.now();
  let lastBodyText = '';
  let lastErrorMessage = '';
  const snapshotEval = `() => ({ groupTrigger: !!document.querySelector('[data-testid="toolbar-create-trigger-group"]'), bodyText: document.body.innerText })`;
  ...
  throw new Error(`Timed out waiting for group page on ${frontendBaseUrl}. Last body text snippet: ${lastBodyText.slice(0, 200)}${errorSuffix}`);
}

async function waitForDialogOpen(timeoutMs = 20000) {
  const startedAt = Date.now();
  while (Date.now() - startedAt < timeoutMs) {
    const result = await runPlaywrightCli(['eval', `() => ({ open: !!document.querySelector('[data-testid="group-create-dialog"]'), flow: !!document.querySelector('[data-testid="new-group-flow-card"]'), form: !!document.querySelector('[data-testid="new-group-form"]') })`], { raw: true });
    const snapshot = parsePlaywrightJson(result.stdout);
...
async function waitForAdvancedStrategyExpanded(targetSelector, timeoutMs = 2000) {
  const waitEval = `() => new Promise(function(resolve) { var selector = ${JSON.stringify(targetSelector)}; var startedAt = Date.now(); var timeoutMs = ${timeoutMs}; function snapshot() { var item = document.querySelector(selector); var content = item?.querySelector('[data-slot="accordion-content"]'); return { found: !!item, state: item?.getAttribute('data-state') || '', contentVisible: !!content, contentText: content?.textContent?.trim() || '', inputCount: content?.querySelectorAll('input[type="number"]').length || 0, helpButtons: content?.querySelectorAll(${JSON.stringify(helpButtonSelector)}).length || 0 }; } function poll() { var current = snapshot(); if (current.state === 'open' && current.contentVisible) { resolve(current); return; } if (Date.now() - startedAt >= timeoutMs) { resolve(current); return; } window.setTimeout(poll, 50); } poll(); })`;
...
async function main() {
  ...
  const tempDir = mkdtempSync(path.join(tmpdir(), 'octopus-group-create-browser-'));
  ...
  try {
    ...
    await seedAuthenticatedGroupPage();

    smokeLog('waiting for group page');
    const groupPage = await ensureGroupPageReady();
    assert.ok(groupPage.groupTrigger, 'group create trigger should exist on group page');

    smokeLog('opening create dialog');
    await runPlaywrightCli(['click', '[data-testid="toolbar-create-trigger-group"]']);
    await waitForDialogOpen();

    const desktopResult = await runPlaywrightCli(['eval', `() => ({ width: window.innerWidth, dialog: !!document.querySelector('[data-testid="group-create-dialog"]'), flowTitle: document.querySelector('[data-testid="new-group-flow-card"]')?.textContent?.trim() || '', namingSection: !!document.querySelector('[data-testid="new-group-naming-section"]'), modeSection: !!document.querySelector('[data-testid="new-group-mode-section"]'), modelSection: !!document.querySelector('[data-testid="new-group-model-picker-section"]'), selectedSection: !!document.querySelector('[data-testid="new-group-selected-section"]'), helpButtons: document.querySelectorAll('[data-testid="group-create-dialog"] ${helpButtonSelector}').length })`], { raw: true });
    ...
    smokeLog('expanding advanced strategy section');
    await runPlaywrightCli(['click', '[data-testid="group-advanced-strategy-trigger"]']);
    const expanded = await waitForAdvancedStrategyExpanded('[data-testid="new-group-advanced-strategy-item"]');
    ...
    await runPlaywrightCli(['hover', '[data-testid="group-create-dialog"] button[data-help-hint-trigger="true"]']);
    ...
    const mobileResult = await runPlaywrightCli(['eval', `() => ({ width: window.innerWidth, dialogWidth: document.querySelector('[data-testid="group-create-dialog"]')?.getBoundingClientRect().width ?? 0, bodyWidth: document.body.scrollWidth, viewport: document.documentElement.clientWidth, advancedTriggerWidth: document.querySelector('[data-testid="group-advanced-strategy-trigger"]')?.getBoundingClientRect().width ?? 0 })`], { raw: true });
    ...
    console.log(JSON.stringify({
      ...
      mobileBodyWidth: mobile.bodyWidth,
      mobileViewport: mobile.viewport,
      mobileWidth: mobile.width,
      result: 'group-create-browser-smoke passed',
    }, null, 2));
  } finally {
    try {
