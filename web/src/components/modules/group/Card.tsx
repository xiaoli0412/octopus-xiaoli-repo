            ...
            <MorphingDialogDescription className="flex-1 min-h-0 overflow-hidden">
                <GroupEditor
                    ...
                    initial={{
                        ...
                        retry_rounds: group.retry_rounds ?? 0,
                        retry_delay_ms: group.retry_delay_ms ?? 0,
                        failover_window_sec: group.failover_window_sec ?? 0,
                        race_after_fails: group.race_after_fails ?? 0,
                        race_concurrency: group.race_concurrency ?? 0,
                        members: displayMembers,
                    }}
                    idPrefix={`edit-group-${group.id ?? 'unknown'}`}
...
export function GroupCard({ group }: { group: Group }) {
    ...
    const displayMembers = useMemo((): SelectedMember[] =>
        [...(group.items || [])]
            ...
            .map((item) => ({
                ...
                channel_name: channelNameByKey.get(modelChannelKey(item.channel_id, item.model_name)) ?? t('channelFallbackName', { id: item.channel_id }),
                item_id: item.id,
                weight: item.weight,
            })),
    ...
    const handleSubmitEdit = useCallback((values: GroupEditorValues, onDone?: () => void) => {
        ...
        const nextRetryRounds = values.retry_rounds ?? 0;
        const nextRetryDelayMs = values.retry_delay_ms ?? 0;
        const nextFailoverWindowSec = values.failover_window_sec ?? 0;
        const nextRaceAfterFails = values.race_after_fails ?? 0;
        const nextRaceConcurrency = values.race_concurrency ?? 0;
        ...
        if (nextRetryRounds !== (group.retry_rounds ?? 0)) payload.retry_rounds = nextRetryRounds;
        if (nextRetryDelayMs !== (group.retry_delay_ms ?? 0)) payload.retry_delay_ms = nextRetryDelayMs;
        if (nextFailoverWindowSec !== (group.failover_window_sec ?? 0)) payload.failover_window_sec = nextFailoverWindowSec;
        if (nextRaceAfterFails !== (group.race_after_fails ?? 0)) payload.race_after_fails = nextRaceAfterFails;
        if (nextRaceConcurrency !== (group.race_concurrency ?? 0)) payload.race_concurrency = nextRaceConcurrency;
