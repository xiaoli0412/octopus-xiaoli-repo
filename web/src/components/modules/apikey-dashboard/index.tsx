...
import { formatNumberByLocale, getNextLocale } from '@/lib/locale';
    ...
    const copyWithToast = useCallback(
        async (text: string, label: string) => {
            try {
                ...
                toast.success(t('copiedModel', { label }));
                return true;
            } catch {
                toast.error(t('error'));
                return false;
                                ...
                                {maxCost > 0 && (
                                    <div className="mt-4">
                                        ...
                                        <div className="flex justify-between text-sm text-muted-foreground mt-1">
                                            <span>0</span>
                                            <span>{formatNumberByLocale(maxCost, locale, { minimumFractionDigits: 2, maximumFractionDigits: 2 })} $</span>
                                        </div>
                                    </div>
                                )}
                            </div>
