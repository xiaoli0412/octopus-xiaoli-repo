...
export function VirtualizedGrid<T>({
    ...
    const effectiveGap = useMemo(() => {
        if (containerWidth < BREAKPOINTS.sm) return Math.min(gap, 10);
        if (containerWidth < BREAKPOINTS.md) return Math.min(gap, 12);
        return gap;
    }, [containerWidth, gap]);
    ...
    const rowVirtualizer = useVirtualizer({
        ...
        estimateSize: () => estimateItemHeight + effectiveGap,
                ...
                {rowCount === 0 ? null : (
                    <div className="relative w-full" style={{ height: `${rowVirtualizer.getTotalSize()}px` }}>
                        {virtualRows.map((virtualRow) => {
                            ...
                            const rowPaddingBottom = virtualRow.index === itemRowCount - 1 && !hasFooterRow ? 0 : effectiveGap;
