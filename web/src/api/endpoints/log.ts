...
export interface RelayLog {
    ...
    cache_read_tokens: number;   // 缓存读取 Token
    cache_write_tokens: number;  // 缓存写入 Token
    ftut: number;                // 首字时间(毫秒)
...
export async function exportLogs(params: LogExportParams = {}) {
    const query: Record<string, string | number | boolean> = {};
    if (params.format) query.format = params.format;
    if (typeof params.start_time === 'number') query.start_time = params.start_time;
    if (typeof params.end_time === 'number') query.end_time = params.end_time;
    if (typeof params.limit === 'number') query.limit = params.limit;

    const response = await apiClient.raw('/api/v1/log/export', query);
    const blob = await response.blob();
    const disposition = response.headers.get('Content-Disposition') || '';
    const fileNameMatch = disposition.match(/filename="?([^";]+)"?/i);
    const fileName = fileNameMatch?.[1] || `octopus-logs.${params.format ?? 'jsonl'}`;
    ...
    useEffect(() => {
        ...
        const connect = async () => {
            try {
				...
				const eventSource = new EventSource(buildApiUrl('/api/v1/log/stream', { token }));
