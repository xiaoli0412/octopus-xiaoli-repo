...
- 安装脚本封装为 `main` 入口，支持 `OCTOPUS_INSTALL_SH_SOURCE_ONLY=1` 安全加载函数，便于本地验证端口与数据目录决策逻辑。
- 新增 `scripts/test-install-functions.sh`，覆盖 Docker Hub 镜像禁用、显式数据目录优先、复用现有容器数据挂载、同名容器端口复用等升级关键分支。
- README / README_zh 的 Docker 镜像示例同步更新到 `v1.19.11`。

## 1.19.10 - 2026-06-08

### 🔧 线上稳定性修复

- 兼容 `genai-prices` 新版 `prices` 数组与 tiered/base 价格结构，修复启动后价格同步因 JSON 结构变化持续告警的问题。
- 一键安装脚本在未显式设置 `OCTOPUS_DATA_DIR` 时会复用现有 `octopus` 容器的 `/app/data` 挂载目录；升级时不再因为默认 `./data` 生成空库。
- 源码 Docker 构建回退路径会写入 `.env` 固定端口、数据目录和容器名，避免后续手动 `docker compose up -d` 回到错误数据目录。

### 📚 安装说明收口

- README / README_zh 同步说明升级数据目录复用逻辑，并把镜像示例更新到 `v1.19.10`。

## 1.19.9 - 2026-06-08

### 🔐 安全与错误处理收口

- 首次管理员密码改为随机生成并写入启动日志；仍可通过 `OCTOPUS_ADMIN_PASSWORD` 显式指定，README / 中文 README / 微信说明同步更新，避免继续引导固定默认密码。
- 用户缓存增加并发读写锁，降低登录、改密和强制首次改密路径的竞态风险。
- 管理端多个 handler 改为返回泛化错误文案，减少内部错误细节在 API 响应中泄露。

### 🔧 运行稳定性与仓库整理

- PostgreSQL 初始化清理语句失败时写入告警，不再静默忽略；Anthropic outbound 包名从历史拼写 `authropic` 统一为 `anthropic`。
- 计费计算抽出共享函数，并保护非 Anthropic 请求中缓存 token 大于 prompt token 时的负数成本边界。
- 恢复前端和文档仍引用的验证脚本，移除误提交的审计临时二进制，保留仓库归档清理结果。

### 🎨 管理界面继续收紧

- 设置、运维、AI 自动化和上游管理页面继续压缩空白、统一卡片密度和标题口径，减少大块提示占位。
- 上游管理页加强紧凑分层展示，渠道、分组、价格等入口之间的视觉间距更稳定。

## 1.19.8 - 2026-06-06

### 🐳 安装脚本输出污染修复

- 修复重装端口检测日志写入 stdout，导致 `docker compose` 收到非法端口值的问题。
- `docker pull` 输出现在不会污染镜像变量；GHCR 可拉取与源码回退两条路径都保持干净参数。

## 1.19.7 - 2026-06-06

### 🐳 README 重装端口修复

- 安装脚本现在会识别 `1088` 是否由现有 `octopus` 容器占用；同名容器重装或升级时复用原端口，不再误判端口占用并跳到 `18080`。
- 继续保留非同名进程/容器占用端口时的自动避让逻辑，避免覆盖用户其他服务。

## 1.19.6 - 2026-06-06

### 🎨 上游下拉菜单修复

- 上游管理页与渠道“从上游添加”页不再使用浏览器原生白底下拉，改为统一 Radix Select 暗色弹层，修复暗色主题下白底浅字、选中态发灰、滚动列表难读的问题。
- 渠道追加列表和登录方式列表增加最大高度滚动，渠道数量较多时不再撑出大块系统弹层；移动端仍保持单列可点选。
- 补齐原生 `select option` 的暗色主题兜底样式，避免后续遗留选择框再次出现浅色弹层和低对比度文字。

## 1.19.5 - 2026-06-06

### 🐳 Docker 安装说明收口

- README / README_zh 将一键安装脚本调整为 Docker 首选路径，避免用户直接复制 GHCR `docker run` 后在受限主机上遇到 `denied`。
- 记录 2026-06-06 服务器实测结果：GHCR 拉取失败后，安装脚本成功回退到 `v1.19.4` 源码 Docker 构建，并在 `1088` 端口启动健康容器。
- 默认安装版本提升到 `1.19.5`，Dockerfile、安装脚本、前后端版本展示和部署示例同步更新。

## 1.19.4 - 2026-06-06

### ✨ 独立上游管理

- 新增与渠道、分组、价格同级的 `上游` 页面，支持 New API、sub2API 与 OpenAI 兼容站点统一接入；渠道页只保留“从上游添加”的轻入口，导入后仍在普通渠道列表中正常管理。
- 新增上游站点、凭据、Key 快照、分组快照与中转价格快照数据层；站点详情按概览、连接、Key、分组、模型、中转价格和刷新记录分层展示，桌面与手机端保持紧凑布局。
- 上游账号密码仅用于一次登录换 token，不长期保存密码；管理 token / access key 加密入库，详情返回时统一脱敏，避免服务层和 API 层泄露密文。

### 🔧 上游探测与渠道联动

- 上游探测补齐 New API / sub2API 多接口序列和 OpenAI 兼容 `/v1/models` 兜底，支持读取模型、Key、分组、余额、订阅、刷新状态和上游价格候选。
- 上游应用到渠道时会写入上游来源引用、Key 来源、allowed models 与请求分类；上游导入渠道可直接参与能力库存、分组路由、测试和统计。
- 新增定时刷新任务，默认每 12 小时刷新上游快照和中转价格；自动刷新不会删除或覆盖本地渠道 Key，只有开启同步时才追加新能力。

### 💰 官方价与中转价双口径

- 价格页新增中转价摘要，单卡只展示 2 条核心来源，更多价格通过上游页搜索和滚动明细查看；官方计价与中转计价分开保存，不再互相覆盖。
- 统计链路新增 gateway / official 双估算口径，实际渠道优先匹配中转价，官方价继续作为对照估算，便于同时观察真实成本和官方成本。
- 上游价格探测补齐 sub2API `/api/v1/pricing`、`/api/v1/model/pricing` 等路径，并更新价格页静态守门，确保三列紧凑布局和中转价联动同时保留。

## 1.19.3 - 2026-06-06

### ✨ 能力库存与上游接入

- 新增统一能力库存口径，打通渠道声明模型、渠道 Key 模型限制、请求分类、分组候选和 API 令牌支持模型，避免“底层能用但页面读不到”或“页面可选但底层不可用”。
- New API / sub2API / OpenAI 兼容上游接入迁移到渠道模块，支持管理 Token、Access Key、账号密码换取 token 等多种方式探测，并读取模型、Key、分组、余额、订阅和上游价格候选。
- 上游导入会把可导入 Key 写入普通渠道，并同步 allowed models、请求能力、分组备注和模型价格候选；管理 token 与脱敏 Key 不会被误写入渠道密钥。

### 🔧 价格、缓存与运维口径

- 价格链路新增上游来源字段与缓存支持语义，导入上游价格时作为中转站点价格候选保存，不覆盖官方价格口径。
- 运维缓存统计区分可缓存与不可缓存模型，不支持缓存的模型不再进入缓存率分母，减少 embedding / audio / image 等模型造成的误判。
- 日志、运维、API 参考和能力库存继续收口到真实可路由模型，减少协议不匹配和模型候选漂移。

### 🎨 管理界面收口

- 设置页移除上游接入大块内容，减少大面积空白；渠道页新增轻量“上游接入”入口，检查结果以紧凑摘要和分层明细展示。
- 上游接入结果只在摘要中显示核心模型与价格数量，其余内容通过滚动明细查看；桌面与手机端均避免横向溢出，并为移动端底部导航预留操作空间。
- AI 自动化、运维、设置等页面继续减少常驻提示与冗长说明，统一卡片密度和客观标题。

## 1.19.2 - 2026-05-31

### ✨ 首页与管理台布局收口

- 首页统计区继续压缩并重排：趋势区补齐更紧凑的热力图样式，排行榜改成更适合横屏三列观察的卡片式排行，渠道页与价格页新增 `正常 / 紧凑` 视图切换并默认维持三列密度，移动端仍保持可读。
- 分组创建 / 分组设置弹窗去掉大段教学文案和无效提示，只保留实际可操作的工作区；模型选择区改为始终可见，单个渠道下模型过多时改成卡片内滚动查看，避免整个弹窗被超长列表撑爆。
- 日志卡片补齐 `cache read / cache write` 令牌展示，并把相关信息与金额、输入输出 token 放在同一区域，便于排查缓存命中与写入成本。

### 🔧 更新链路与本地开发收口

- 新增 `/api/v1/update/status`，设置页可以区分“有新版本但当前平台不支持自更新”的场景；Windows 下会明确提示只能手动替换二进制或容器镜像，不再弹出笼统更新失败。
- `internal/update` 现在显式拒绝 Windows 自更新，同时补齐 handler 与单测覆盖，避免把不支持的平台继续走到半途中断。
- Windows 本地脚本统一切到仓库内 `use-node-env.ps1` 的固定 Node / pnpm 入口；`build-win.ps1`、`dev-win.ps1` 和验证链一起对齐，减少宿主机 PATH 漂移造成的构建失败。

### 🧠 模型价格与缓存统计补强

- 模型价格页继续沿用本轮紧凑三列布局，同时保留模型卡对 `厂商 / 版本 / 后缀` 等归一化结果的承载能力，方便后续继续扩充价格覆盖。
- OpenAI 风格响应里的 `prompt_tokens_details.cached_tokens` 与 `cache_creation_input_tokens` 已打通到内部 usage 解析、日志持久化和前端展示链路，缓存读取 / 写入消耗现在可以在日志与模型价格相关视图中统一观察。
- 默认 HTTP transport 增加响应头超时保护，并补充回归测试，降低上游无响应时长期悬挂的风险。

## 1.19.1 - 2026-05-23

### ✨ 运维工作台

- 新增独立 `运维` 一级界面，并将其放到 `AI 自动化` 上方；页面内按 `总览 / 模型 / 渠道 / Key / 缓存 / IP` 分层，支持二级下钻查看趋势与最近调用明细。
- 运维页支持按维度切换 `折线图 / 柱状图 / 点状图`，统一展示总成功率、模型成功率、渠道成功率、渠道 Key 成功率、用户分发 Key 成功率，以及最近 12 小时的 IP 调用统计。
- 缓存观测已并入运维页：按模型、渠道、渠道 Key、分发 Key 统计 `缓存命中率 / 缓存率 / 缓存读取 / 缓存写入`，并与成功率、延迟一起联动展示。

### 🔧 后端统计与性能收口

- 新增轻量 `OpsMetricBucket` 聚合桶表，按 5 分钟窗口记录运维面板所需的成功/失败、延迟、缓存读取、缓存写入和缓存命中计数，避免为运维页重复扫描全量日志。
- 中继完成时会同步写入运维聚合数据，并补齐 `ClientIP` 采集，用于 IP 访问调用统计；渠道与渠道 Key 统计按尝试级别计数，模型、分发 Key 与总览按请求级别计数。
- 新增固定 `12 小时` 运维清理任务，只保留运维页当前展示所需的聚合窗口；近期二级明细继续复用最近 12 小时日志数据做过滤，不额外引入第二套重型明细存储。

### ✨ 模型价格识别补强

- 价格 / 模型链路补齐了更强的模型名归一化能力，开始显式识别厂商、版本和后缀，并把这些解析结果回填到模型价格列表。
- 模型价格同步从单一 `models.dev` 扩展为多源融合：保留原有源，同时并入 LiteLLM、genai-prices 和 llm-prices 的公开数据，补强模型价格覆盖率。
- 模型价格页现在会在列出模型前先补齐“全局已引用模型”清单，覆盖渠道声明模型、分组项模型、渠道允许模型、路由目标覆盖模型和分发 Key 限制模型，减少“模型存在但价格页缺失”的情况。

### 🔧 运维页收口

- 进一步收紧 `运维` 页的布局节奏，统一标题口径，压缩空白区，避免在横屏宽度下出现明显错位和挤压。
- 空数据状态改成更明确的客观提示，避免空图表和空明细把页面撑散；移动端明细改为卡片式展示，减少横向拥挤。
## 1.18.8 - 2026-05-10
