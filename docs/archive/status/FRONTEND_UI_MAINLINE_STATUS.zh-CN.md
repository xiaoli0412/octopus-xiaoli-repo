...
- 设置页备份 / 导入：`已部分完成`
  ...
  - 备份页现已把 rollback domains 编辑真正接回 history 主流程：`Backup.tsx` 新增独立 rollback scope editor，preview / selected rollback 会共享同一组 `routing / models / api_keys / settings / stats / logs` scopes；scope 一旦变化，当前 rollback preview 会立即失效，避免 compare 面板继续展示旧 scope 的结果。与此同时，若所有 rollback scopes 都被取消，preview / rollback 仍会回退为 full snapshot restore，不会向接口发送空 scope 对象。
  - 备份页现在还会在 selective import 开着但没有 active scopes 时直接禁用导入按钮，并提示必须先选 scope，避免把空选择提交到 import 路径。
- 备份页兼容性详情现已把“更细粒度的导入冲突引导”真正落地：在 signal list 之外新增“下一步处理建议”卡，按阻断风险、缺失渠道/模型、凭证重绑定、模型映射修正、路由/策略差异五类给出可执行指引；同一详情区也会直接展开 `credential_rebind_targets`、`invalid_route_targets`、`model_policy_diffs` 等结构化明细，不再只停在总数提示。
- 备份页兼容性详情现在还会把“复核路由与策略差异”这条建议真正落到对应细项：`route_preview_warnings`、`route_preview_diffs` 与 `alias_preview_mappings` 已接入同一详情区的结构化明细，用户在看见建议卡后可以直接继续检查路由预警、候选链路差异和别名映射预览，而不必只依赖摘要计数或跳回别处找上下文。
- `2026-05-07` 又继续把 replace/map 最后一块 repo-local pending seam 收掉：当前 `Backup.tsx` 已把 `compatibility.replace_pruned_channels / groups / settings / llm_infos / api_keys` 直接接进兼容性详情区，并在 signal list 与 guidance 中明确提示“当前替换会清理哪些记录”；与此同时，导入页过时的 `Import migration tooling / 导入补强项` 占位入口已移除，不再把已闭环的 replace/map 引导继续显示成 pending。对应 `tsc --noEmit`、`verify-backup-logic.mjs` 与 `verify-backup-component.cjs` 已通过，因此 Backup repo-local 剩余缺口继续收敛为 live browser/CDP 证据与宿主级 Vitest blocker，而不是导入冲突引导本身。
- 备份页兼容性详情里残留的 `缺失渠道 / Provider` 已改回 `缺失渠道 / 供应商`，并在 no-browser verification script 中补了中文主显示断言，避免中文界面再次泄漏英文主文案。
  ...
  - 本轮 rollback domains 编辑的 repo-local 验证已由 `tsc --noEmit`、`verify-backup-component.cjs` 与 `verify-backup-logic.mjs` 覆盖；但 `Backup.test.tsx` 对应的 `vitest/vite` 入口在当前 Windows 宿主上仍继续命中 `spawn EPERM`，所以这部分仍按环境 blocker 记录，不判定为备份页回归。
- 本轮导入冲突引导补强的 repo-local 验证同样已由 `tsc --noEmit`、`verify-backup-component.cjs` 与 `verify-backup-logic.mjs` 覆盖；`Backup.test.tsx` 与 `backup-logic.test.ts` 的用例也已同步更新，但 `Vitest/Vite` 在当前 Windows 宿主上依旧命中 `spawn EPERM`，因此这部分继续按环境 blocker 记录，而不是新的备份页回退。
- 本轮 route warning / route diff / alias preview 细节消费链的 repo-local 验证同样已由 `tsc --noEmit`、`verify-backup-component.cjs` 与 `verify-backup-logic.mjs` 覆盖；新增的 `Backup.test.tsx` 与 `backup-logic.test.ts` 断言已落盘，但 `Vitest/Vite` 入口依旧复现宿主级 `spawn EPERM`，因此继续记为环境 blocker，而不是本轮细节补丁回归。
...
- `2026-05-03` 同一条 backup 验证子线继续把入口从“只有共享 CLI wrapper”补到“共享 CLI wrapper + shared-root CDP thin forwarder”：当前 `verify-backup-browser-smoke.ps1` 已公开 `-Driver cdp|cli`，新增 `verify-backup-browser-smoke-cdp.ps1` 继续像 `group-create / channel-page / settings-help` 一样 forward 到共享 `verify-channel-create-browser-smoke-cdp.ps1`，而 `verify-backup-browser-smoke-cdp.mjs` 则直接承接 Backup 页的导入 dry-run、Apply Same Import、history、rollback preview 与 `375px` 检查。对应 `check-only -Driver cdp`、`-BrowserPath` 兼容调用与 `verify-browser-smoke-wrapper-alignment.mjs` 都已通过，因此 Backup 页当前剩余 browser gap 已从“缺入口”收口为“还缺 live CDP 绿灯证据”。
- `2026-05-03` 同一条 backup 验证子线继续把真实 smoke 覆盖面补到最近几轮 Phase 6 新增交互：当前 `verify-backup-browser-smoke.mjs` 与 `verify-backup-browser-smoke-cdp.mjs` 不再只验证 dry-run/apply/history/preview 面板存在，而是会进一步检查 rollback scope editor 的默认 full-restore 摘要、关闭 `routing / stats / logs` 后的 narrowed summary、scope 变化触发 preview invalidation、再次 preview 后 `backup-rollback-preview-meta-scope` 的 `models,api_keys,settings -> models,api_keys` 回显，以及 rollback route-diff compare 面板与 compare row。对应 repo-local `--check`、`check-only -Driver cli|cdp` 与 `verify-backup-component.cjs` 已通过；但 live `self-start -Driver cli` 仍稳定命中 Playwright CLI `spawn EPERM`，live `self-start -Driver cdp` 仍稳定命中 `9222/json/version` 启动超时，因此当前 Backup 页 browser/runtime 剩余缺口已更精确地收敛为宿主级 live blocker，而不是 smoke 覆盖面不足。
...
- `2026-05-03` 同池 wrapper 守门也已同步吸收这条 Backup CDP 增量：`verify-browser-smoke-wrapper-alignment.mjs` 现在会把 `verify-backup-browser-smoke-cdp.ps1` 纳入 inventory，并固定检查 `NodeSmokeScript = 'scripts/verify-backup-browser-smoke-cdp.mjs'`、`backup-browser-smoke-cdp passed`、`SmokeLabel = 'backup'`、`BrowserPath` 透传，以及默认 `attached-session + page-lifecycle-runtime` 参数组合。这样后续若 Backup CDP wrapper 无声回退成复制版脚本或漂移到错误场景脚本，`test:screenshot-no-browser` 会先在 repo-local 失败，而不是等到下一轮再靠文档回忆。
...
补充口径（2026-05-08）：Docker Hub 已从项目官方安装路径中移除。前端主线后续若涉及安装说明、引导文案、验收文档或入口提示，必须默认对齐 GHCR 官方镜像、显式私有 / 代理镜像，以及安装脚本在 GHCR 拉取失败时的源码支撑 Docker 构建；不得再向用户展示 Docker Hub 为默认或官方来源。
