# 2026-04-25 Phase G Backup Compatibility Toggle Closure
...
- add the next smallest page-level contract for `backup-compatibility-toggle` in both the component test and the repo-local verification script
- keep the write scope narrow and avoid touching unrelated backup logic
- verify with repo-local typecheck and backup verification

## 本次硬规则

- 继续沿用当前 Phase G backup selector-contract 主线
- 本轮只补 `backup-compatibility-toggle` 的 selector 契约，不扩到更大的备份逻辑
...
- `Backup.test.tsx` 对 `backup-compatibility-toggle` 有稳定断言
...
1. 在 backup 页面测试中补充 `backup-compatibility-toggle` 的 root-level selector 断言。
2. 在 `scripts/verify-backup-component.cjs` 的对应路径中补充同一 selector 断言。
...
- 构建是否通过：待验证
- 测试是否通过：待验证
- 本次使用了哪些本地资源 / skills / 记忆上下文：待填写
- 本次使用的本地资源 / skills / 记忆上下文分别提供了什么结论：待填写
- 本次使用了哪些子 agent 及其结论：未使用
- 子 agent 分工、负责范围与产出摘要：未使用子 agent，原因是本轮任务很小且写作用域可控

## 本轮补充结果

- 实际完成：`scripts/verify-backup-component.cjs` 的 map 分支已从文本触发 `Show n` 收紧为直接点击 `backup-compatibility-toggle`。
- 未完成：`Backup.test.tsx` 没有新增代码改动，本轮没有再扩大测试面。
- 验证：仅完成静态盘点与差异确认，未能执行 `node scripts/verify-backup-component.cjs` 或 `tsc --noEmit`。
- 阻塞：本机 Node 脚本执行仍受 `ncrypto::CSPRNG(nullptr, 0)` 宿主级故障影响。
- 下一步：继续留在 Phase G backup selector-contract 主线，优先找下一个纯 selector 的最小闭环，避免再回到文本触发。
