# 2026-05-01 Backend Task Responses Function-Call-Arguments Done ItemID Silence Closure

## 1. 任务信息

- 任务名称：Responses `function_call_arguments.done` 仅 `item_id` 时保持静默
- 日期：2026-05-01
- 当前阶段：`LLM Gateway canonical refactor / Phase B Responses done-event identity recovery`
...
- 上一个相关 worklog：`docs/archive/worklog/worklog/2026-05-01-backend-task-responses-output-item-done-itemid-silence-closure.md`
- 本次任务目标：沿同一 Phase B transformer compatibility 主线，补一条 focused regression，锁定 `response.function_call_arguments.done` 在只携带 `item_id`、没有任何新增 `name/arguments/call_id` 时应保持静默，不再误触发额外 fallback chunk。
- 本次已盘点本地资源：
  - `AGENTS.md`
...
- 候选任务 1：补 `function_call_arguments.done` 仅 `item_id` 的静默回归。
...
- 新增一条 focused regression，验证 `response.function_call_arguments.done` 只有 `item_id` 时不会多吐 fallback chunk。
- 相关 transformer 测试通过。
- `git diff --check` 无新增 patch 问题。

## 5. 结果记录

- 已完成：新增 `TestResponseOutboundTransformStreamSkipsToolCallArgumentsDoneWithItemIDOnly`，锁定 `response.function_call_arguments.done` 只有 `item_id`、没有新增 `name/arguments/call_id` 时保持静默。
...
2. 若继续留在 tool-call lane，更适合看 `output_item.done` / `function_call_arguments.done` 在更多空字段组合下的 consumer 假设，而不是切到更大范围 relay/race。
